...
fname <- c("/Volumes/Macintosh_HD_3/genetics/genenetwork2/gene_names_check_v3.Rnw")
...
# March 23, 2018: The ENTREZIDs from  n = 5 randomly sampled of rows from each brain area agree perfectly with the same entrezgene IDs looked up by hand using Gene Cards.
  ...
  # Replace Septin 10 symbol with SEPT10
  
  sept10.indx.pre <- lapply(major.Area[i], function(x,i) {which(x$Symbol == 'Septin 10')},i)

    for (im in 1:4)  {
       
       major.Area[[im]]$Symbol[unlist(sept10.indx.pre)] <- c('SEPT10')
  }
  
  sept10.indx.post <- lapply(major.Area[i], function(x,i) {which(x$Symbol == 'Septin 10')},i)
  
  
  symb.with.space <- lapply(major.Area[i], function(x,i) {which(grepl(' ',x$Symbol))},i)
  
  # All of the symbols with spaces are of the form 'Septin n' where n is a number with the exception of 'Selenoprotein 15' which is 'SEP15'
  
  unique.gene.space <- unique(c(major.Area[[1]]$Symbol[unlist(symb.with.space[[1]])],
          major.Area[[2]]$Symbol[unlist(symb.with.space[[2]])],
          major.Area[[3]]$Symbol[unlist(symb.with.space[[3]])],
          major.Area[[4]]$Symbol[unlist(symb.with.space[[4]])]))
  
  unique.gene.space <- unique.gene.space[c(1,2,3,6,4,5,7,9,10,8)]
  replace.gene.space <- data.frame(orig=c("Septin 2","Septin 3","Septin 4","Septin 5","Septin 6",
                                   "Septin 7","Septin 9","Septin 11","Septin 12","Selenoprotein 15"),repl=c('SEPT2',
                                              'SEPT3',
                                              'SEPT4',
                                              'SEPT5',
                                              'SEPT6',
                                              ...
                                              'SEPT12',
                                              'SEP15'))
 replace.space.indx <- list()
   
 for (yi in 1:4) {
  
       replace.space.indx[[yi]] <- match(major.Area[[yi]]$Symbol[unlist(symb.with.space[[yi]])],
                                         replace.gene.space[,1])
  }
  
 for (yi in 1:4)   {
     major.Area[[yi]]$Symbol[unlist(symb.with.space[[yi]])] <- as.character(replace.gene.space[unlist(replace.space.indx[[yi]]),2])
 }
...
# Replace ORF with orf when it is meant to indicate open reading frame because official symbols # use 'orf'.
  
  which.ORF.indx <- lapply(major.Area[i], function(x,i) which(grepl('ORF',x$Symbol,ignore.case=FALSE)),i)
  sum.ORF.indx <- data.frame(ORD2orf=c(length(which.ORF.indx[[1]]),
                                       length(which.ORF.indx[[2]]),
                                       length(which.ORF.indx[[3]]),
                                       length(which.ORF.indx[[4]])))
  row.names(sum.ORF.indx) <- stand.Col.names
  print(sum.ORF.indx)
...
for (im in 1:4)  {
     ...
     major.Area[[im]]$Symbol[unlist(which.ORF.indx[[im]])] <- sub('ORF','orf',major.Area[[im]]$Symbol[unlist(which.ORF.indx[[im]])],ignore.case=FALSE)

}
# Done April 2, 2018 and wrote out new data files
#            ORD2orf
# Prefrontal       0
# Cerebellum       0
# Temporal         0  
...
<<>>=
# Write interim results to primary data files
...
# #  # Changes were previously written out to the primary data sets and the master data set
# #  
...
# First try to use the ENTREZIDs to get the missing Chr numbers/names

  un.Chr <- list()
  
  un.Chr <- lapply(major.Area[i], function(x,i) which(x$loc == 'Un'))
  unique.un.entrez <- c(length(unique(major.Area[[1]]$ENTREZID[unlist(un.Chr[[1]])])),
                        length(unique(major.Area[[2]]$ENTREZID[unlist(un.Chr[[2]])])),
                        length(unique(major.Area[[3]]$ENTREZID[unlist(un.Chr[[3]])])),
                        length(unique(major.Area[[4]]$ENTREZID[unlist(un.Chr[[4]])]))) 

 num.un <- data.frame(Number_Un_Chr=unlist(lapply(un.Chr[i], function(x,i) length(x)),i),
                      Unique_ENTREZIDs=unlist(unique.un.entrez))
 num.un.tab <- xtable(num.un,
                      caption='Number of rows with Chromosome as Un in each brain area.')
 
 num.un[5,1:2] <- c(sum(num.un$Number_Un_Chr),sum(num.un$Unique_ENTREZIDs))
 
 row.names(num.un) <- c(row.names(num.un)[1:4],'sum')
 
 #April 2, 2018
#     Number_Un_Chr Unique_ENTREZIDs
# pf           2055             1847
# cb           2047             1833
# tm           2043             1822
# po           2027             1806
# sum          8172             7308
 ...
 # Use ENTREZIDs to get Chr=Un entries
 
   miss.un.returned <- lapply(major.Area[i], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','chromosome_name'), filters ='entrezgene', values =x$ENTREZID[unlist(y)], mart = ensembl)}, i, un.Chr[i])
   
   no.chr.in.name <- lapply(miss.un.returned[i], function(x,i) which(!grepl('CHR',x$chromosome_name)),i)
   
   temp.no.chr <- list()
   unique.entrez.temp <- list()
   
   for (im in 1:4)  {
   
   temp.no.chr[[im]] <- match(major.Area[[im]]$ENTREZID[unlist(un.Chr[[im]])],as.character(miss.un.returned[[im]]$entrezgene[unlist(no.chr.in.name[[im]])]))
   
   }
   
   major.Area[[1]]$loc[unlist(un.Chr[[1]])] <- as.character(miss.un.returned[[1]]$chromosome_name[unlist(no.chr.in.name[[1]])])[unlist(temp.no.chr[[1]])]
   
   major.Area[[2]]$loc[unlist(un.Chr[[2]])] <- as.character(miss.un.returned[[2]]$chromosome_name[unlist(no.chr.in.name[[2]])])[unlist(temp.no.chr[[2]])]
   
   major.Area[[3]]$loc[unlist(un.Chr[[3]])] <- as.character(miss.un.returned[[3]]$chromosome_name[unlist(no.chr.in.name[[3]])])[unlist(temp.no.chr[[3]])]
   
   major.Area[[4]]$loc[unlist(un.Chr[[4]])] <- as.character(miss.un.returned[[4]]$chromosome_name[unlist(no.chr.in.name[[4]])])[unlist(temp.no.chr[[4]])]
   
   un.Chr.post <- lapply(major.Area[i], function(x,i) which(x$loc == 'Un'))
   
   unique.un.entrez.post <- c(length(unique(major.Area[[1]]$ENTREZID[unlist(un.Chr.post[[1]])])),
                        length(unique(major.Area[[2]]$ENTREZID[unlist(un.Chr.post[[2]])])),
                        length(unique(major.Area[[3]]$ENTREZID[unlist(un.Chr.post[[3]])])),
                        length(unique(major.Area[[4]]$ENTREZID[unlist(un.Chr.post[[4]])]))) 

 num.un <- data.frame(Number_Un_Chr=unlist(lapply(un.Chr.post[i], function(x,i) length(x)),i),
                      Unique_ENTREZIDs=unlist(unique.un.entrez.post))
...
# April 4, 2018: There are no entries that have 'Un' for the chromosome number/name
 # This is because 'Un' was replaced by NA whern the chromsosome name could not be found
 
# Check for NA in chromosome names (loc)
  miss.Tab <- list()
...
# miss.Symb   0   0  0   0  
# miss.entrez 0   0  0   0  
# miss.loc    104 98 104 100    Reduced from approximately 2500 'Un' 
  
  prefront.Data$loc <- major.Area[[1]]$loc
  cbell.Data$loc <- major.Area[[2]]$loc
  tempor.Data$loc <- major.Area[[3]]$loc
  pons.Data$loc <- major.Area[[4]]$loc
...
@ 
 ...
 << bp1.00 >>=
  ...
  # Now find missing start positions; this shows up as bp = 1.00
 
  miss.bp <- list()
  
  miss.bp <- lapply(major.Area[i], function(x,i) which(x$bp == 1.00))
  
  num.miss.bp <- lapply(miss.bp[i], function(x,i) length(x),i)
  
  num.miss.bp.tab <- data.frame(Number_missing_start_positions=unlist(num.miss.bp))
  
  # April 43, 2018
#    Number_missing_start_positions
# pf                           2113
# cb                           2101
# tm                           2098
# po                           2084  
...
# The start positions need to be divided by 1Mb (1,000,000) to make tyhem consistent with the existing entries.
     ...
     #                             miss.bp.returned[[w]]$entrezgene))
       ...
       match.bp.indx[[w]] <- match(major.Area[[w]]$ENTREZID[unlist(miss.bp[[w]])],
                                  as.character(miss.bp.returned[[w]]$entrezgene))   
  ...
  # Check for NA in chromosome starting positions (bp)
  miss.Tab <- list()
...
# miss.Symb   0   0  0   0  
# miss.entrez 0   0  0   0  
# miss.loc    104 98 104 100
# miss.bp     104 99 104 100
  
  # Both the chromosome name and starting position missing counts were reduced from approximately 2500 to 100.
 
  prefront.Data$bp <- major.Area[[1]]$bp
  cbell.Data$bp <- major.Area[[2]]$bp
  tempor.Data$bp <- major.Area[[3]]$bp
  pons.Data$bp <- major.Area[[4]]$bp 
...
<< recover_Chr_names>>=
# Try to find missing chromosome names using gene Symbols rather than entrezgene.
  
  na.loc.indx <- lapply(major.Area[ib], function(x,i) which(is.na(x$loc)),i)
     na.bp.indx <- lapply(major.Area[ib], function(x,i) which(is.na(x$bp)),i)
     
     na.loc.returned <- lapply(major.Area[ib], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','chromosome_name'), filters ='hgnc_symbol', values =as.character(x$Symbol[unlist(y)]), mart = ensembl)}, i,na.loc.indx[ib])
     
     na.loc.returned.entrez <- lapply(major.Area[ib], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','chromosome_name'), filters ='entrezgene', values =as.character(x$ENTREZID[unlist(y)]), mart = ensembl)}, i,na.loc.indx[ib])
     
# Try to extract the Chromosome name/number from the returned values that are formatted as 'CHR_HSCHR'

na.loc.returned.temp <- list(symb=rep(NA,4),
                             entrez=rep(NA,4))
temp.Init <- list(symbol=rep(NA,4),
                  entrezg=rep(NA,4))

na.loc.returned.temp$symb <- lapply(na.loc.returned[ib], function(x,ib) sub('CHR_HSCHR','',x$chromosome_name), ib)

na.loc.returned.temp$entrez <- lapply(na.loc.returned.entrez[ib], function(x,ib) sub('CHR_HSCHR','',x$chromosome_name), ib)

temp.Init$symbol <- lapply(na.loc.returned.temp$symb[ib], function(x,ib)   grep('^[X-Y]|^[1-9]?|^[1]{1}[0-9]{1}|^[2]{1}[0-2]{1}[^:punct:]',x,value=TRUE),ib)
temp.Init$entrezg <- lapply(na.loc.returned.temp$entrez[ib], function(x,ib)   grep('^[X-Y]|^[1-9]?|^[1]{1}[0-9]{1}|^[2]{1}[0-2]{1}[^:punct:]',x,value=TRUE),ib)


temp.Init$symbol <- lapply(temp.Init$symbol[ib], function(x,ib) substr(x,1,2), ib)
temp.Init$entrezg <- lapply(temp.Init$entrezg[ib], function(x,ib) substr(x,1,2), ib)

temp.Init$symbol <- lapply(temp.Init$symbol[ib], function(x,ib) sub('\\_','',x), ib)
temp.Init$entrezg <- lapply(temp.Init$entrezg[ib], function(x,ib) sub('\\_','',x), ib)

temp.Init$symbol <- lapply(temp.Init$symbol[ib], function(x,ib) sub('CH',NA,x),ib)
temp.Init$entrezg <- lapply(temp.Init$entrezg[ib], function(x,ib) sub('CH',NA,x),ib)

loc.as.CH.num <- data.frame(total_returnedSymb=sapply(na.loc.returned[ib], function(x,ib) length                               (x$chromosome_name), ib),
                           CH_returnedSymb = sapply(na.loc.returned[ib], function(x,ib) length                                (grep('CHR_',x$chromosome_name)),ib),
                           CH2NAleftSymb = sapply(temp.Init$symbol[ib], function(x,ib) num.NA(x                                ),ib),
                           total_returnedEntrez=sapply(na.loc.returned.entrez[ib], function(x,ib                                )length(x$chromosome_name), ib),
                           CH_returnedEmntrez = sapply(na.loc.returned.entrez[ib], function(x,ib                                )length(grep('CHR_',x$chromosome_name)),ib),
                           CH2NAleftEntrez = sapply(temp.Init$entrezg[ib], function(x,ib) num.NA                                (x),ib))

loc.as.CH.num

loc.as.CH.num.tab <- xtable(loc.as.CH.num,
                              caption='Query statistics for additional Chromosome names using gene Symbols or ENTREZID before and after reformatting of CHR_xxx and extraction of Chromosome names from the returned values.')
...
\Sexpr{loc.as.CH.num.tab}

<< check_NA_refromatted_extracted_Chr_names >>=                            
...
temp.no.chr <- list()
# loc.match.Symb.indx <- list(symb=rep(NA,4),
#                             entrezg=rep(NA,4))
loc.match.Symb.indx <- list(indx=c(),
                            symb=c(),
                            entrez=c(),
                            loc=c(),
                            chrName=c())

loc.match.entrez.indx <- list(indx=c(),
                            symb=c(),
                            entrez=c(),
                            loc=c(),
                            chrName=c())
...
# loc.match.Symb.indx <- list()
# loc.match.entrez.indx <- list()

found.Chr.names <- list(symb=rep(NA,4),
                            entrezg=rep(NA,4))
add.Chr.names <- data.frame(Additional_start_Pos = rep(NA,4))

for (im in 1:4)  {
     
     # loc.match.Symb.indx$symb[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.loc.indx[[im]])],na.loc.returned[[im]]$hgnc_symbol)
     
     loc.match.Symb.indx$indx[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.loc.indx[[im]])],na.loc.returned[[im]]$hgnc_symbol)
     loc.match.Symb.indx$symb[[im]] <- na.loc.returned[[im]]$hgnc_symbol[unlist(loc.match.Symb.indx$indx[[im]])]
     loc.match.Symb.indx$entrez[[im]] <- na.loc.returned[[im]]$entrezgene[unlist(loc.match.Symb.indx$indx[[im]])]
     loc.match.Symb.indx$loc[[im]] <- na.loc.returned[[im]]$chromosome_name[unlist(loc.match.Symb.indx$indx[[im]])]
     loc.match.Symb.indx$chrName[[im]] <- temp.Init$symbol[[im]][unlist(loc.match.Symb.indx$indx[[im]])]
     
     # loc.match.entrez.indx[[im]] <- match(major.Area[[im]]$ENTREZID[unlist(na.loc.indx[[im]])],na.loc.returned.entrez[[im]]$entrezgene)
     
     loc.match.entrez.indx$indx[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.loc.indx[[im]])],na.loc.returned.entrez[[im]]$hgnc_symbol)
     loc.match.entrez.indx$symb[[im]] <- na.loc.returned.entrez[[im]]$hgnc_symbol[unlist(loc.match.entrez.indx$indx[[im]])]
     loc.match.entrez.indx$entrez[[im]] <- na.loc.returned.entrez[[im]]$entrezgene[unlist(loc.match.entrez.indx$indx[[im]])]
     loc.match.entrez.indx$loc[[im]] <- na.loc.returned.entrez[[im]]$chromosome_name[unlist(loc.match.entrez.indx$indx[[im]])]
     loc.match.entrez.indx$chrName[[im]] <- temp.Init$entrezg[[im]][unlist(loc.match.entrez.indx$indx[[im]])]
     
     found.Chr.names$symb[[im]] <- sum(!is.na(loc.match.Symb.indx$chrName[[im]]))
     
     found.Chr.names$entrezg[[im]] <- sum(!is.na(loc.match.entrez.indx$chrName[[im]]))

     } # end brain area loop

add.Chr.names <- data.frame(Symbol_query = unlist(found.Chr.names$symb),
                            entrez_query = unlist(found.Chr.names$entrezg))
row.names(add.Chr.names) <- stand.Col.names
add.Chr.names.tab <- xtable(add.Chr.names,
                            caption='Additional Chromosome names recovered using Symbol Query, column 1, and entrezgene Query, column 2')

@

\Sexpr{add.Chr.names.tab}


<< union_Symb_entrez_returns >>=

found.Chr <- list()   # Will hold the union of chromosome names from the two queries
# found.Chr can be used to replace the missing, NA, values in major.Area and in the oprimary data files.

for (im in 1:4)  {
     
     found.Chr[[im]] <- pmin(temp.Init$symbol[[im]][unlist(loc.match.Symb.indx$indx[[im]])],temp.Init$entrezg[[im]][unlist(loc.match.entrez.indx$indx[[im]])],na.rm=TRUE)
     
     major.Area[[im]]$loc[unlist(na.loc.indx[[im]])] <- found.Chr[[im]] 
...
missing.loc <- data.frame(Missing_Chr_names=unlist(lapply(major.Area[ib], function(x,ib) num.NA(x$loc), ib)))

missing.loc.tab <- xtable(missing.loc,
                          caption='Remaining numbers of missing chromosome name rows by brain area.')
...
<< find_miss_start_pos_bp >>=
     
     na.bp.indx <- lapply(major.Area[ib], function(x,i) which(is.na(x$bp)),i)
     
     na.bp.returned <- lapply(major.Area[ib], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','start_position'), filters ='hgnc_symbol', values =as.character(x$Symbol[unlist(y)]), mart = ensembl)}, i,na.bp.indx[ib])
     
     na.bp.returned.entrez <- lapply(major.Area[ib], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','start_position'), filters ='entrezgene', values =as.character(x$ENTREZID[unlist(y)]), mart = ensembl)}, i,na.bp.indx[ib])
     

# na.bp.returned.temp <- list(symb=rep(NA,4),
#                              entrez=rep(NA,4))
temp.Init <- list(symbol=rep(NA,4),
                  entrezg=rep(NA,4))

temp.Init$symbol <- lapply(na.bp.returned[ib], function(x,ib) x$start_position, ib)
temp.Init$entrezg <- lapply(na.bp.returned.entrez[ib], function(x,ib) x$start_position, ib)
...
# rows that had NA under the bp variable
#  pf  cb  tm  po 
# 104  99 104 100 

temp.no.chr <- list()

bp.match.Symb.indx <- list(indx=c(),
                            symb=c(),
                            entrez=c(),
                            start.Pos=c(),
                            bp=c())

bp.match.entrez.indx <- list(indx=c(),
                            symb=c(),
                            entrez=c(),
                            start.Pos=c(),
                            bp=c())
...
found.start.Pos <- list(symb=rep(NA,4),
                            entrezg=rep(NA,4))
add.start.Pos <- data.frame(Additional_start_Pos = rep(NA,4))

for (im in 1:4)  {
     
# returned by Symbol query     
     bp.match.Symb.indx$indx[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.bp.indx[[im]])],na.bp.returned[[im]]$hgnc_symbol)
     bp.match.Symb.indx$symb[[im]] <- na.bp.returned[[im]]$hgnc_symbol[unlist(bp.match.Symb.indx$indx[[im]])]
     bp.match.Symb.indx$entrez[[im]] <- na.bp.returned[[im]]$entrezgene[unlist(bp.match.Symb.indx$indx[[im]])]
     bp.match.Symb.indx$start.Pos[[im]] <- na.bp.returned[[im]]$start_position[unlist(bp.match.Symb.indx$indx[[im]])]
     bp.match.Symb.indx$bp[[im]] <- temp.Init$symbol[[im]][unlist(bp.match.Symb.indx$indx[[im]])]

# returned by entrezgene query     
     bp.match.entrez.indx$indx[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.bp.indx[[im]])],na.bp.returned.entrez[[im]]$hgnc_symbol)
     bp.match.entrez.indx$symb[[im]] <- na.bp.returned.entrez[[im]]$hgnc_symbol[unlist(bp.match.entrez.indx$indx[[im]])]
     bp.match.entrez.indx$entrez[[im]] <- na.bp.returned.entrez[[im]]$entrezgene[unlist(bp.match.entrez.indx$indx[[im]])]
     bp.match.entrez.indx$start.Pos[[im]] <- na.bp.returned.entrez[[im]]$start_position[unlist(bp.match.entrez.indx$indx[[im]])]
     bp.match.entrez.indx$bp[[im]] <- temp.Init$entrezg[[im]][unlist(bp.match.entrez.indx$indx[[im]])]
     
     found.start.Pos$symb[[im]] <- sum(!is.na(bp.match.Symb.indx$start.Pos[[im]]))
     
     found.start.Pos$entrezg[[im]] <- sum(!is.na(bp.match.entrez.indx$start.Pos[[im]]))

     } # end brain area loop

add.start.Pos <- data.frame(Symbol_query = unlist(found.start.Pos$symb),
                            entrez_query = unlist(found.start.Pos$entrezg))
row.names(add.start.Pos) <- stand.Col.names
add.start.Pos.tab <- xtable(add.start.Pos,
                            caption='Additional starting positions recovered using Symbol Query, column 1, and entrezgene Query, column 2')

@

\Sexpr{add.start.Pos.tab}


<< union_Symb_entrez_returns >>=

found.bp <- list()   # Will hold the union of chromosome names from the two queries
# found.bp can be used to replace the missing, NA, values in major.Area and in the oprimary data files.

for (im in 1:4)  {
     
     found.bp[[im]] <- pmin(temp.Init$symbol[[im]][unlist(bp.match.Symb.indx$indx[[im]])]/1000000,temp.Init$entrezg[[im]][unlist(bp.match.entrez.indx$indx[[im]])]/1000000,na.rm=TRUE)
     
     major.Area[[im]]$bp[unlist(na.bp.indx[[im]])] <- found.bp[[im]] 
...
missing.bp <- data.frame(Missing_start_Pos=unlist(lapply(major.Area[ib], function(x,ib) num.NA(x$bp), ib)))

missing.bp.tab <- xtable(missing.bp,
                          caption='Remaining numbers of missing starting positions rows by brain area.')
