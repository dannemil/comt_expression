...
<< recover_Chr_names>>=
# Try to find missing chromosome names and staert positions using gene Symbols rather than entrezgene.
  
  na.loc.indx <- lapply(major.Area[i], function(x,i) which(is.na(x$loc)),i)
     na.bp.indx <- lapply(major.Area[i], function(x,i) which(is.na(x$bp)),i)
     
     na.loc.returned <- lapply(major.Area[i], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','chromosome_name'), filters ='hgnc_symbol', values =as.character(x$Symbol[unlist(y)]), mart = ensembl)}, i,na.loc.indx[i])
     
     na.loc.returned.entrez <- lapply(major.Area[i], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','chromosome_name'), filters ='entrezgene', values =as.character(x$ENTREZID[unlist(y)]), mart = ensembl)}, i,na.loc.indx[i])
...
found.Chr.names <- list(symb=rep(NA,4),
                            entrezg=rep(NA,4))
add.Chr.names <- data.frame(Additional_Chr_names = rep(NA,4))
...
<< >>=
temp.no.chr <- lapply(major.Area[ib], function(x,ib,y,z) match(x$Symbol[unlist(y)],z$hgnc_symbol), ib, na.loc.indx[ib], na.loc.returned[ib])
...
temp.no.chr <- list()
   ...
   temp.no.chr[[im]] <- match(major.Area[[im]]$Symbol[unlist(na.loc.indx[[im]])],as.character(na.loc.returned[[im]]$hgnc_symbol[unlist(no.chr.in.name[[im]])]))
   
   }
# This only recovers an additional 10 - 15 Chr names in each brain area
     
     
     
          
     na.bp.returned <- lapply(major.Area[i], function(x,i,y) {getBM(attributes=c('hgnc_symbol','start_position'), filters ='hgnc_symbol', values =as.character(x$Symbol[unlist(y)]), mart = ensembl)}, i,na.bp.indx[i])
