...
### Change the positions that it looks for the motifs in

mot.site <- c(0,1)      #### 0,1 looks at -1, +1 TSS

motif.Inr <- c('[CGT][AT]CA[CGT][AT]',
               '[CT][CT]A[ACGT][AT][CT][CT]',
               'TG'         #### Just look for CA at -1, +1      '[C/T][A/G]'
               )
loc.lims.t <- mot.site
loc.lims <- 1000 + loc.lims.t

motif.hits.loci.pos <- list()
num.Inr.pos <- matrix(rep(NA,16),ncol=4)
for (im in ib)   {
     
     num.Inr.pos[im,4] <- c(length(proms.pos[[im]]))
     at.least <- matrix(rep(NA,3*length(proms.pos[[im]])),ncol=3)
     loc.hits <- list()
...
num.Inr.pos.pc <- matrix(rep(NA,7*4),ncol=7)
num.Inr.pos.pc[,c(1,3,5,7)] <- num.Inr.pos[,c(1,2,3,4)]
num.Inr.pos.pc[,c(2,4,6)] <- round(100*num.Inr.pos.pc[,c(1,3,5)]/num.Inr.pos.pc[,7],1)
...
num.Inr.pos.f <- data.frame(motif1=c(paste(num.Inr.pos.pc[,1],
                         ...
                         Genes=num.Inr.pos.pc[,7])


colnames(num.Inr.pos.f) <- c("Inr1a",
                            "Inr2b",
                            "Inr3c",
                           'Number of Genes')
...
num.Inr.pos.tab <- xtable(num.Inr.pos.f,
                                caption=c("Number (percentage) of pos COMT correlated genes in each brain area having at least one instance of each of three different consensus intiator element (Inr) sequences in the region from -100 to + 50 nucleotides relative to the transcription start site TSS (adjacent to +1)"))
...
### Change the negitions that it looks for the motifs in
...
num.Inr.neg <- matrix(rep(NA,16),ncol=4)
for (im in ib)   {
     
     num.Inr.neg[im,4] <- c(length(proms.neg[[im]]))
     at.least <- matrix(rep(NA,3*length(proms.neg[[im]])),ncol=3)
     loc.hits <- list()
...
num.Inr.neg.pc <- matrix(rep(NA,7*4),ncol=7)
num.Inr.neg.pc[,c(1,3,5,7)] <- num.Inr.neg[,c(1,2,3,4)]
num.Inr.neg.pc[,c(2,4,6)] <- round(100*num.Inr.neg.pc[,c(1,3,5)]/num.Inr.neg.pc[,7],1)
...
num.Inr.neg.f <- data.frame(motif1=c(paste(num.Inr.neg.pc[,1],
                         ...
                         Genes=num.Inr.neg.pc[,7])


colnames(num.Inr.neg.f) <- c("Inr1a",
                            "Inr2b",
                            "Inr3c",
                           'Number of Genes')
...
num.Inr.neg.tab <- xtable(num.Inr.neg.f,
                                caption=c("Number (percentage) of neg COMT correlated genes in each brain area having at least one instance of each of three different consensus intiator element (Inr) sequences in the region from -100 to + 50 nucleotides relative to the transcription start site TSS (adjacent to +1)"))
