...
# Write out the file_name, unique ID and date/time to '.Rscriptnames'
          ...
          rec.script(Rin=eval(fname),ui=eval(prog.Name))
...
mot.site <- c(0,1)      #### 0,1 looks at -1, +1 TSS

### Look for the consensus Y-R [C/T][G/A]   at -1, +1 TSS
motif.Inr <- c('CA',
               'CG',
               'TA',
               'TG'
...
num.Inr.all <- matrix(rep(NA,20),ncol=5)
num.with.motif <- list()

for (im in ib)   {
     
     num.with.motif.temp <- c(rep(NA,length(motif.Inr)))
     num.Inr.all[im,5] <- c(length(proms.all[[im]]))
     at.least <- matrix(rep(NA,4*length(proms.all[[im]])),ncol=4)
     loc.hits <- list()
     
     for (ai in 1:length(motif.Inr))      {
               
          loc.hits.temp <- c(0)
     
          grep.temp <- c(NA)
          
          for (kh in 1:length(proms.all[[im]]))     {
          
               cat(paste('brain area ',im,' motif ',motif.Inr[ai],' gene ',kh,' \n',sep=''))
          ...
          num.with.motif.temp[ai] <- sum(at.least[,ai])
          
     
}  # motif type loop ai
     
     motif.hits.loci.all[[im]] <- loc.hits
     num.with.motif[[im]] <- round(num.with.motif.temp/length(proms.all[[im]]),4)
     num.Inr.all[im,1] <- sum(at.least[,1])
     num.Inr.all[im,2] <- sum(at.least[,2])
     num.Inr.all[im,3] <- sum(at.least[,3])
     num.Inr.all[im,4] <- sum(at.least[,4])
...
num.Inr.all.pc <- matrix(rep(NA,9*4),ncol=9)
num.Inr.all.pc[,c(1,3,5,7,9)] <- num.Inr.all[,c(1,2,3,4,5)]
num.Inr.all.pc[,c(2,4,6,8)] <- round(100*num.Inr.all.pc[,c(1,3,5,7)]/num.Inr.all.pc[,9],1)
...
num.Inr.all.f <- data.frame(motif1=c(paste(num.Inr.all.pc[,1],
                         ...
                         ' (',round(num.Inr.all.pc[,6],3),') ',sep='')),
                          motif4=c(paste(num.Inr.all.pc[,7],
                         ' (',round(num.Inr.all.pc[,8],3),') ',sep='')),
                         Genes=num.Inr.all.pc[,9])


colnames(num.Inr.all.f) <- c(motif.Inr,
                           'Number of Genes')
...
num.Inr.all.tab <- xtable(num.Inr.all.f,
                                caption=c("Number (percentage) of all COMT-correlated genes in each brain area having the dinucleotide sequence shown as the column header in positions [-1, +1] relative to the transcription start site TSS. These dinucleotides represent one of the consensus sequences (Y-R) for the Initiator element in the promoter region."),digits=c(0,0,0,0,0,0))

print(num.Inr.all.tab,type='latex')

@


<<initiator.element.Positive.subset>>=
...
#### fasta.by.lett_pos.genes[[1]][[1]][1:10] only has the 51 letters from -40 to +10 for pos genes in the COMT project in pf
...
motif.hits.loci.pos <- list()
num.Inr.pos <- matrix(rep(NA,20),ncol=5)
num.with.motif <- list()

for (im in ib)   {
     
     num.with.motif.temp <- c(rep(NA,length(motif.Inr)))
     num.Inr.pos[im,5] <- c(length(proms.pos[[im]]))
     at.least <- matrix(rep(NA,4*length(proms.pos[[im]])),ncol=4)
     loc.hits <- list()
     ...
     for (ai in 1:length(motif.Inr))      {
          ...
          for (kh in 1:length(proms.pos[[im]]))     {
     ...
     motif.hits.loci.pos[[im]] <- loc.hits
     num.with.motif[[im]] <- round(num.with.motif.temp/length(proms.pos[[im]]),4)
     num.Inr.pos[im,1] <- sum(at.least[,1])
     num.Inr.pos[im,2] <- sum(at.least[,2])
     num.Inr.pos[im,3] <- sum(at.least[,3])
     num.Inr.pos[im,4] <- sum(at.least[,4])
...
for (im in ib)    {
     for (jy in 1:4)    {
          ...
          motif.hits.loci.pos[[im]][[jy]] <- subset(motif.hits.loci.pos[[im]][[jy]],
                                        motif.hits.loci.pos[[im]][[jy]] >= -100)
...
num.Inr.pos.pc <- matrix(rep(NA,9*4),ncol=9)
num.Inr.pos.pc[,c(1,3,5,7,9)] <- num.Inr.pos[,c(1,2,3,4,5)]
num.Inr.pos.pc[,c(2,4,6,8)] <- round(100*num.Inr.pos.pc[,c(1,3,5,7)]/num.Inr.pos.pc[,9],1)

num.Inr.pos.f <- data.frame(motif1=c(paste(num.Inr.pos.pc[,1],
                         ' (',round(num.Inr.pos.pc[,2],3),') ',sep='')),
                         motif2=c(paste(num.Inr.pos.pc[,3],
                         ' (',round(num.Inr.pos.pc[,4],3),') ',sep='')),
                         motif3=c(paste(num.Inr.pos.pc[,5],
                         ' (',round(num.Inr.pos.pc[,6],3),') ',sep='')),
                          motif4=c(paste(num.Inr.pos.pc[,7],
                         ' (',round(num.Inr.pos.pc[,8],3),') ',sep='')),
                         Genes=num.Inr.pos.pc[,9])


colnames(num.Inr.pos.f) <- c(motif.Inr,
                           'Number of Genes')
row.names(num.Inr.pos.f) <- stand.Col.names.aug



num.Inr.pos.tab <- xtable(num.Inr.pos.f,
                                caption=c("Number (percentage) of strongest positively COMT-correlated genes in each brain area having the dinucleotide sequence shown as the column header in positions [-1, +1] relative to the transcription start site TSS. These dinucleotides represent one of the consensus sequences (Y-R) for the Initiator element in the promoter region."),digits=c(0,0,0,0,0,0))

print(num.Inr.pos.tab,type='latex')

@

<<initiator.element.Negative.subset>>=
...
#### fasta.by.lett_neg.genes[[1]][[1]][1:10] only has the 51 letters from -40 to +10 for neg genes in the COMT project in pf

#### proms.neg[[1]] has the COMT genes as fasta formatted 1600-character entries.
...
motif.hits.loci.neg <- list()
num.Inr.neg <- matrix(rep(NA,20),ncol=5)
num.with.motif <- list()

for (im in ib)   {
     
     num.with.motif.temp <- c(rep(NA,length(motif.Inr)))
     num.Inr.neg[im,5] <- c(length(proms.neg[[im]]))
     at.least <- matrix(rep(NA,4*length(proms.neg[[im]])),ncol=4)
     loc.hits <- list()
     ...
     for (ai in 1:length(motif.Inr))      {
          ...
          for (kh in 1:length(proms.neg[[im]]))     {
     ...
     motif.hits.loci.neg[[im]] <- loc.hits
     num.with.motif[[im]] <- round(num.with.motif.temp/length(proms.neg[[im]]),4)
     num.Inr.neg[im,1] <- sum(at.least[,1])
     num.Inr.neg[im,2] <- sum(at.least[,2])
     num.Inr.neg[im,3] <- sum(at.least[,3])
     num.Inr.neg[im,4] <- sum(at.least[,4])
...
for (im in ib)    {
     for (jy in 1:4)    {
          ...
          motif.hits.loci.neg[[im]][[jy]] <- subset(motif.hits.loci.neg[[im]][[jy]],
                                        motif.hits.loci.neg[[im]][[jy]] >= -100)
...
num.Inr.neg.pc <- matrix(rep(NA,9*4),ncol=9)
num.Inr.neg.pc[,c(1,3,5,7,9)] <- num.Inr.neg[,c(1,2,3,4,5)]
num.Inr.neg.pc[,c(2,4,6,8)] <- round(100*num.Inr.neg.pc[,c(1,3,5,7)]/num.Inr.neg.pc[,9],1)

num.Inr.neg.f <- data.frame(motif1=c(paste(num.Inr.neg.pc[,1],
                         ' (',round(num.Inr.neg.pc[,2],3),') ',sep='')),
                         motif2=c(paste(num.Inr.neg.pc[,3],
                         ' (',round(num.Inr.neg.pc[,4],3),') ',sep='')),
                         motif3=c(paste(num.Inr.neg.pc[,5],
                         ' (',round(num.Inr.neg.pc[,6],3),') ',sep='')),
                          motif4=c(paste(num.Inr.neg.pc[,7],
                         ' (',round(num.Inr.neg.pc[,8],3),') ',sep='')),
                         Genes=num.Inr.neg.pc[,9])


colnames(num.Inr.neg.f) <- c(motif.Inr,
                           'Number of Genes')
row.names(num.Inr.neg.f) <- stand.Col.names.aug



num.Inr.neg.tab <- xtable(num.Inr.neg.f,
                                caption=c("Number (percentage) of strongest negatively COMT-correlated genes in each brain area having the dinucleotide sequence shown as the column header in negitions [-1, +1] relative to the transcription start site TSS. These dinucleotides represent one of the consensus sequences (Y-R) for the Initiator element in the promoter region."),digits=c(0,0,0,0,0,0))

print(num.Inr.neg.tab,type='latex')

@

<<plots.CA.CG.TA.TG.posVneg>>=
### Set up long data frame

Inr.YR.posVneg <- data.frame(posCA=num.Inr.pos.pc[,2],
                             posCG=num.Inr.pos.pc[,4],
                             posTA=num.Inr.pos.pc[,6],
                             posTG=num.Inr.pos.pc[,8],
                             negCA=num.Inr.neg.pc[,2],
                             negCG=num.Inr.neg.pc[,4],
                             negTA=num.Inr.neg.pc[,6],
                             negTG=num.Inr.neg.pc[,8])

Inr.YR.posVneg.long <- stack(Inr.YR.posVneg)
Inr.YR.posVneg.long[,3] <- c(rep(c('Positive','Negative'),each=16))
Inr.YR.posVneg.long[,4] <- c(rep(c(stand.Col.names.aug),8))
Inr.YR.posVneg.long[,5] <- c(rep(rep(c('CA','CG','TA','TG'),each=4),2))
Inr.YR.posVneg.long[,6] <- c(rep(c(1,-1),each=16))*Inr.YR.posVneg.long[,1]


colnames(Inr.YR.posVneg.long) <- c('PercentGenes','ind','CorrValence',
                                   'BrainArea','Motif',
                                   'flipNeg')

Inr.YR.posVneg.long$CorrValence <- as.factor(Inr.YR.posVneg.long$CorrValence)
Inr.YR.posVneg.long$BrainArea <- as.factor(Inr.YR.posVneg.long$BrainArea)
Inr.YR.posVneg.long$Motif <- as.factor(Inr.YR.posVneg.long$Motif)

               

ya.plot <- ggplot(data=Inr.YR.posVneg.long, aes(x=Motif,fill=CorrValence)) +
     facet_wrap( ~ BrainArea,ncol=2)  +


     geom_bar(stat = "identity", data = Inr.YR.posVneg.long,aes(y = flipNeg)) +
     # geom_text(data = Inr.YR.posVneg.long, aes(y = flipNeg, 
     #                          label = PercentGenes), 
     #                          size = 2.5, hjust = c(-1.0)) +
     ...
     xlab("Motif") +
     guides(fill = FALSE) +
     geom_hline(yintercept = c(0),size=c(0.5),color='black',linetype='dashed')  +
     annotate("text", x = c('TG'), y = c(-30),
              label = c('Negative subset'),
              size = 2.5,
              color='black')  +
     annotate("text", x = c('TG'), y = c(25),
              label = c('Positive subset'),
              size = 2.5,
              color='black')
