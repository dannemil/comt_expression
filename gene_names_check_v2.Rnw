...
# April 4, 2018: There are no entries that have 'Un' for the chromosome number/name
 # This is because 'Un' wasm replaced by NA whern the chromsosome name could not be found
...
<< hgnc >>=
# Try to replace 'LOCxxx' gene symbols with approved symbols
# Note: the hgnc_complete_database.xlsx has no approved gene symbols that begin with 'LOCxx'
  
  # 1. Use the HGNC database with the entrezgene to get the approved symbol to try to replace the
    #  'LOCxxxx' gene symbols
  
# Indices (rows) of entries with 'LOCxxx' as gene symbol
  LOC.indx <- lapply(major.Area[i], function (x,i) genes.Are.locs(x,i), i)
 
LOC.num <- data.frame(Genes_with_LOCsymbols = unlist(lapply(LOC.indx[i], function(x,i) length(x), i))) 

LOC.num

#    Genes_with_LOCsymbols
# pf                   167
# cb                   170
# tm                   166
# po                   165
...
LOC.symbol.returned <- lapply(major.Area[i], function(x,i,y) {getBM(attributes=c('entrezgene','hgnc_symbol','ensembl_gene_id'), filters ='entrezgene', values = as.character(x$ENTREZID[unlist(y[i])]), mart = ensembl)}, i,LOC.indx)
...
# Try to find missing chromosome names and staert positions using gene Symbols rather than entrezgene.
  ...
  na.loc.indx <- lapply(major.Area[i], function(x,i) which(is.na(x$loc)),i)
     ...
     # no.chr.in.name <- lapply(na.loc.returned[i], function(x,i) which(!grepl('CHR',x$chromosome_name)),i)
     

# Try to extract the Chromosome name/number from the returned values that are formatted as 'CHR_HSCHR'

na.loc.returned.temp <- list()
temp.Init <- list()

na.loc.returned.temp[[1]] <- sub('CHR_HSCHR','',na.loc.returned[[1]]$chromosome_name)

temp.Init[[1]] <- grep('^[X-Y]|^[1-9]?|^[1]{1}[0-9]{1}|^[2]{1}[0-2]{1}[^:punct:]',na.loc.returned.temp[[1]],value=TRUE)

temp.Init[[1]] <- substr(temp.Init[[1]],1,2)
temp.Init[[1]] <- sub('\\_','',temp.Init[[1]])
temp.Init[[1]][which(temp.Init[[1]] == 'CH')] <- NA
temp.Init[[1]][which(temp.Init[[1]] == 'KI')] <- NA
num.NA(temp.Init[[1]])

temp.no.chr[[1]] <- match(major.Area[[1]]$Symbol[unlist(na.loc.indx[[1]])],as.character(na.loc.returned[[1]]$hgnc_symbol))

as.character(temp.Init[[1]][temp.no.chr[[1]]])
