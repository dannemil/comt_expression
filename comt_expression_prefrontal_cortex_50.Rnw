...
cisplot2neg <- paste(pre.Name,'cisplot2neg.pdf',sep='')
...
top.50.pos <- order.by.P.pos[1:num.Top, ]
top50.pos.Count <- table(top.50.pos[,1])


## This scales the number of probes in the top 50 inversely by the fitted number of probes from the regression of number of probes by chromosome length. It also preserves the absolute number of probes found in the top 50 (~886)
fit.inverse.Scaled <- as.numeric(top50.pos.Count)/(adjust.By.fit[1:23]/sum(adjust.By.fit[1:23]))
fit.inverse.Scaled <- round((fit.inverse.Scaled*(sum(top50.pos.Count)/sum(fit.inverse.Scaled))),0)
...
# 
# num2get <- top50.pos.Count*scaled.Ratio$prop
...
top.50.neg <- order.by.P.neg[1:num.Top, ]
top50.neg.Count <- table(top.50.neg[,1])

scaled.Props <- as.numeric(full.Count)/num.Neg
scaled.top50.Props <- as.numeric(top50.neg.Count)/sum(as.numeric(top50.neg.Count))
...
fit.inverse.Scaled <- as.numeric(top50.neg.Count)/(adjust.By.fit[1:23]/sum(adjust.By.fit[1:23]))
fit.inverse.Scaled <- round((fit.inverse.Scaled*(sum(top50.neg.Count)/sum(fit.inverse.Scaled))),0)
...
hist(comt.Meannegr$corr,xlab=c('Sample correlation negative r only'),
    main=c(''),
    col='gray90',
    20, xlim = rev(range(comt.Meannegr$corr))
    )
