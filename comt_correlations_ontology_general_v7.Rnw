...
hold.Lowest.pvals.sign <- list(neg=hold.Lowest.pvals.neg,
...
fname <- c('top500temp_genes_tabled_by_area_pval_rank_final.tex')
...
prefrontal.top500.tab <- xtable(VEC2MAT(hold.lowest.w.Symbcorsign[[1]]$Symbol,10),
...
prefrontal.top500.ltx <- latex(prefrontal.top500.tab,file='top500temp_genes_tabled_by_area_pval_rank_final.tex',
                         ...
                         table.env=TRUE,
                         center=c('center'),
                         continued=c('Top 500 negitively Correlated Genes in PF Crtx Continued'),
...
cerebellum.top500.tab <- xtable(VEC2MAT(hold.lowest.w.Symbcorsign[[2]]$Symbol,10),
...
cerebellum.top500.ltx <- latex(cerebellum.top500.tab,file='top500temp_genes_tabled_by_area_pval_rank_final.tex',
                         ...
                         table.env=TRUE,
                         center=c('center'),
                         continued=c('Top 500 negitively Correlated Genes in Crblm Continued'),
                         first.hline.double = FALSE,
                         append=TRUE
                     )


temporal.top500.tab <- xtable(VEC2MAT(hold.lowest.w.Symbcorsign[[3]]$Symbol,10),
...
temporal.top500.ltx <- latex(temporal.top500.tab,file='top500temp_genes_tabled_by_area_pval_rank_final.tex',
                         ...
                         table.env=TRUE,
                         center=c('center'),
                         continued=c('Top 500 negitively Correlated Genes in Temp Crtx Continued'),
                         first.hline.double = FALSE,
                         append=TRUE
                     )
...
pons.top500.tab <- xtable(VEC2MAT(hold.lowest.w.Symbcorsign[[4]]$Symbol,10),
...
pons.top500.ltx <- latex(pons.top500.tab,file='top500temp_genes_tabled_by_area_pval_rank_final.tex',
                         ...
                         table.env=TRUE,
                         center=c('center'),
                         continued=c('Top 500 negitively Correlated Genes in Pons Continued'),
                         first.hline.double = FALSE,
                         append=TRUE
                     )
} else {
...
fname <- c('expression_distributions_by_area.pdf')
if (!file.exists(fname))   {
...
} # END conditional chunk execution

@

<<neg_neg_corrs_unique_numbers>>=

if (chunk.Status)    {

# How many (what percentage) of the correlations in the full data (only one probe per gene) in each area are negitive/negative?
# How many (what percentage) of the correlations among the top 500 in each area are negative/negative?
# How many (what percentage) of the correlations unique genes in each area are negitive/negative?

neg.neg.corrs.nums <- data.frame(fullneg=rep(NA,4),
                                 full=rep(NA,4),
                                 top500neg=rep(NA,4),
                                 top500=rep(NA,4),
                                 uniqueneg=rep(NA,4),
...
neg.neg.corrs.nums[,1] <- sapply(area.List.byGene[ib], function(x,ib) sum(x$corsign == '1'), ib)
neg.neg.corrs.nums[,2] <- sapply(area.List.byGene[ib], function(x,ib) dim(x)[1], ib)
neg.neg.corrs.nums[,3] <- sapply(hold.Lowest.pvals[ib], function(x,ib) sum(x$corsign == '1'), ib)
neg.neg.corrs.nums[,4] <- sapply(hold.Lowest.pvals[ib], function(x,ib) dim(x)[1], ib)
neg.neg.corrs.nums[,5] <- sapply(unique.full.data.all[ib], function(x,ib) sum(x$corsign == '1'), ib)
neg.neg.corrs.nums[,6] <- sapply(unique.full.data.all[ib], function(x,ib) dim(x)[1], ib)

### Write out this table and use the hypergeomatric distribution to test for deviations from expected

row.names(neg.neg.corrs.nums) <- stand.Col.names

neg.neg.corrs.nums.tab <- xtable(neg.neg.corrs.nums,
                                 caption=c('Number of negitively COMT-correlated genes in the full sample (gene as unit of analysis) (Column 1), in the 500 most strongly correlated (ranked by p-values) (Column 3), and in the genes among the top 500 only found in the corresponding brain area but not among the top 500 genes in any of the other three areas (Column 5).'))

neg.neg.corrs.nums.ltx <- latex(neg.neg.corrs.nums.tab,file='',
                         caption = c('Number of negitively COMT-correlated genes in the full sample (gene as unit of analysis) (Column 1), in the 500 most strongly correlated (ranked by p-values) (Column 3), and in the genes among the top 500 only found in the corresponding brain area but not among the top 500 genes in any of the other three areas (Column 5).'),
                         caption.loc = c('top'),
                         colheads = c('neg. r full sample',
                                     'Full Sample',
                                     'neg. r top 500 genes',
                                     'Top 500 genes',
                                     'neg. r unique',
                         ...
                         colnamesTexCmd="bfseries",
                         booktabs=TRUE,
                         label=c('tab:num.neg.genes'),
                         na.blank = TRUE,
                         vbar = FALSE,
                         longtable=TRUE,
                         table.env=TRUE,
                         center=c('center'),
                         continued=c('Numbers of negitively Correlated Genes Continued'),
                         first.hline.double = FALSE,
                         append=FALSE
                     )
...
if (!file.exists('hypergeometric_tests_negative_final.tex'))   {

neg.neg.percent <- data.frame(fullneg=rep(NA,4),
                              top500neg=rep(NA,4),
                              uniqueneg=rep(NA,4)
                              )

neg.neg.percent[,1] <- neg.neg.corrs.nums[,1]/neg.neg.corrs.nums[,2]
neg.neg.percent[,2] <- neg.neg.corrs.nums[,3]/neg.neg.corrs.nums[,4]
neg.neg.percent[,3] <- neg.neg.corrs.nums[,5]/neg.neg.corrs.nums[,6]

#   fullneg top500neg uniqueneg
...
#  neg.neg.corrs.nums
#            fullneg  full top500neg top500 uniqueneg unique
...
# top 500 neg against full sample

hypergeom.Tests <- data.frame(top500=rep(NA,4),
                              unique=rep(NA,4))
for (im in ib)   {

hypergeom.Tests[im,1] <- formatC(phyper(neg.neg.corrs.nums$top500neg[im],
                              neg.neg.corrs.nums$fullneg[im],
                              (neg.neg.corrs.nums$full[im] - neg.neg.corrs.nums$fullneg[im]),
                              neg.neg.corrs.nums$top500[im],
                              lower.tail=FALSE,
                              log.p=FALSE
                              ),format = "e", digits = 2)

hypergeom.Tests[im,2] <- formatC(phyper(neg.neg.corrs.nums$uniqueneg[im],
                              neg.neg.corrs.nums$fullneg[im],
                              (neg.neg.corrs.nums$full[im] - neg.neg.corrs.nums$fullneg[im]),
                              neg.neg.corrs.nums$unique[im],
                              lower.tail=FALSE,
                              log.p=FALSE
                              ),format = "e", digits = 2)
...
The data show that\\
\begin{enumerate}
     \item In the full sample with gene as the unit of analysis, there are approximately twice as many genes whose expression is negatively correlated with {\textit {COMT}} expression as negitively correlated genes. This holds independently of brain area. The same is true if probe is used as the unit of analysis instead of gene.
     \item In contrast, among the 500 most strongly {\textit {COMT}}-correlated genes in each brain area, there are approximately twice as many negitively correlated as negatively correlated genes: a complete reversal of the proportions in the full sample.
     ...
     \begin{itemize}
          \item In Cerebellum, Temporal Cortex and Pons, the proportion of negitively correlated genes among these unique sets is similar to what it was in the full set of the 500 top genes in that area. Thast is, there are about twice as many negitively corrleated as negatively correlated genes.
          \item In Prefrontal Cortex, however, this proportion flips so that there are approximately twice as many negatively correlated genes as negitively correlated genes. This is the same proportion that existed in the full set of data rather than in the top 500. 
     \end{itemize}
\end{enumerate}

So the two remarkable aspects of this analysis are 1) that there are twice as many negatively {\textit {COMT}}-correlated genes as negitively correlated genes, but this proportion flips in the 500 most strongly (ranked by p-value) correlated genes in each area, and 2) this 2:1 negitive:negative ratio also holds among the uniquely expressed genes in a given area in all areas except Prefrontal Cortex where the proportion reverses to 1:2 as it was in the full sample of data.
...
# The returned sequence is in the 5' to 3' direction. The start negition in measuring
...
# The same is true for entrezgene 780 in the temporal data set but it is the down stream part that is missing. this is for seq.neg.fasta[[3]][[117]] and the third transcript for 780 has complete sequence information:
...
# match the entrezgene ids of the surviving fasta sequences against the ENTREZIDs of the top 500 genes in order to extract the corsign to add to the fasta id for each sequence.

# The purpose of this is to be able to test for differences in motif content between positively and negatively COMT-correlated genes.
...
# comnege fasta id line to be added before each fasta sequence
...
#     hold.fasta[[]] <- strsplit(unlist(seq.fasta[[1]]),NULL)
     ...
     lett.tab.tata <- matrix(rep(0,length(seq.neg.fasta[[im]])*4),ncol=4)
...
tss.convert <- 1001
#tata.range <- seq(-33,-28,1)   # TATA box check
tata.range <- seq(-4,4,1)   # TSS


tata.range.rel <- tss.convert + tata.range
trl <- tata.range.rel
num.neg.fasta <- seq(1,as.integer(length(fasta.by.lett.neg$pf)/1600),1)
num.neg.fasta <- seq(1,as.integer(length(fasta.by.lett.neg$pf)/1600),1)


neg.lett.freq.tata <- list(pf=matrix(rep(0,length(seq.neg.fasta[[1]])*4),ncol=4),
                      cb=matrix(rep(0,length(seq.neg.fasta[[2]])*4),ncol=4),
                      tm=matrix(rep(0,length(seq.neg.fasta[[3]])*4),ncol=4),
                      po=matrix(rep(0,length(seq.neg.fasta[[4]])*4),ncol=4)
...
lett.tab.tata.neg <- matrix(rep(0,length(trl)*4),ncol=length(trl))
colnames(lett.tab.tata.neg) <- as.character(tata.range)
row.names(lett.tab.tata.neg) <- DNA_BASES


for (kl in num.neg.fasta)  {
     
     for (ik in 1:length(trl))  {
          
          for (ju in 1:4)  {
     
               if (fasta.by.lett.neg$pf[[kl]][ik] == DNA_BASES[ju])  {
                    lett.tab.tata.neg[ju,ik] <- lett.tab.tata.neg[ju,ik] + 1
          }
               else {
     ...
     }  # end sequence index 1..6 (trl)
} # end number of negitive genes

# Make into a PWM

pwm.neg <- matrix(as.integer(lett.tab.tata.neg),ncol=length(trl))
colnames(pwm.neg) <- as.character(tata.range)
row.names(pwm.neg) <- DNA_BASES
ggseqlogo( pwm.neg )

#     neg33   neg32   neg31   neg30   neg29   neg28
# A 0.24768 0.24458 0.24768 0.28793 0.25697 0.24149
# C 0.20124 0.22291 0.26316 0.25697 0.21362 0.27864
# G 0.26006 0.29102 0.25077 0.21981 0.26006 0.23839
# T 0.29102 0.24149 0.23839 0.23529 0.26935 0.24149



# pwm.neg <- data.frame(pwm.neg)
# pwm.neg <- apply(pwm.neg, 2, proportion)
# 
# # pwm.neg <- prop.table(pwm.neg,2)
# pwm.neg <- makePWM(pwm.neg)
# 
# seqLogo(pwm.neg)
...
#     neg33   neg32   neg31   neg30   neg29   neg28
# A 0.29762 0.25595 0.26190 0.22619 0.27976 0.27976
# C 0.21429 0.20833 0.22024 0.16071 0.25000 0.20833
# G 0.23214 0.25595 0.21429 0.25595 0.23214 0.23810
# T 0.25595 0.27976 0.30357 0.35714 0.23810 0.27381
...
# From Top 500 run all, negitive only and negative only
...
num.bp <- c(5)
len.seq <- nchar(substr(promoter.set[[1]][1],1,1000))/num.bp
...
prop.test(c(colSums(comp.cpg.stats[[2]][[1]]$cpg.island)[5],
            colSums(comp.cpg.stats[[3]][[1]]$cpg.island)[5]),
          c(tot.promoters[[2]][1],
            tot.promoters[[3]][1]),
...
#          max.cpg.run[gy,3] <- length(lengths[vals][lengths[vals] >= 200])
     ...
     cpg.dist[[pn]] <- max.cpg.run
}

     cpg.dist.area[[im]] <- cpg.dist
}
@
...
prop.cpg.near.TSS <- c(sum(cpg.dist.area[[4]][[2]][,5])/tot.promoters[[2]][4],
                  sum(cpg.dist.area[[4]][[3]][,5])/tot.promoters[[3]][4])

prop.cpg.near.TSS.test <- prop.test(c(sum(cpg.dist.area[[4]][[2]][,5]),
                                      sum(cpg.dist.area[[4]][[3]][,5])),
                                      c(tot.promoters[[2]][4],
                                        tot.promoters[[3]][4]),
                                    alternative='two.sided')


ks.test(cpg.dist.area[[1]][[2]][,4][cpg.dist.area[[1]][[2]][,4] > 0],
        cpg.dist.area[[1]][[3]][,4][cpg.dist.area[[1]][[3]][,4] > 0],
        alternative = 'two.sided')
...
# Make most.3prime.negition relative to TSS

# df.plot$test <- df.plot$test - 1001

by(df.plot$test,df.plot$corsign,summary)
...
# genes from top500 negitively correlated in Prefrontal, but absent from each of the other areas
...
cpg.df$negVneg <-c(rep('negitive',4*length(perc.cpg$bp[[area.num]])),
...
colnames(num.shared.genes) <- c('negitive in Common',
                                'Negative in Common',
                                'negitive in Common with Negative',
                                'Negative in Common with negitive',
                                'Union(negitive)',
                                'Union(Negative)',
                                'Proportion negitive in Common',
...
row.names(lowest.pvals.corsignXchrom) <- c('negative','negitive','prop. neg.')
...
The most strongly negitively and negatively COMT-correlated genes are distributed proportionately unequally across the 22 autosomes and the X chromosome. There are significantly (p = 0.00034) fewer negatively correlated than negitively correlated genes on Chromosome 1 than would be expected from the overall proportion of negatively COMT-correlated genes across the 500 most strongly correlated genes. The COMT gene is located on Chromosome 22.\\
...
motifLatexTable(main="FG: negitive COMT correlations, BG: negative COMT correlations, Prefrontal motifs", PF.motifs, prefix="negnegPF",plot.pwm= TRUE,summary.cols=c(1,7,8,9))

plotMotif(PF.motifs$motifs[[1]]@match$pattern, logodds=TRUE, entropy=TRUE,has.box=TRUE,
          main = 'Dinucleotide differential motif 1 \n for negitively COMT-correlated versus \n negatively COMT-correlated genes in Prefrontal Cortex')
...
# Hypothesis: the mean number of hits for a given motif will be greater for the negitively correlated genes than for the negatively correlated genes.
# Compare the mean number of motif hits in the negitive vs. negative groups. Do this separately by motif, then combine all 3 motifs and do this with the sum.
...
motifLatexTable(main="FG: negitive pf COMT correlations, BG: negitive cb COMT correlations, Prefrontal vs. Cerebellum motifs", PF.motifs, prefix="negPFvsCB")
...
motifLatexTable(main="FG: negitive COMT correlations, BG: negative COMT correlations, Cerebellum motifs", CB.motifs, prefix="negnegCB")
...
motifLatexTable(main="FG: negitive COMT correlations, BG: negative COMT correlations, Temporal Cortex motifs", TM.motifs, prefix="negnegTM")
...
motifLatexTable(main="FG: negitive COMT correlations, BG: negative COMT correlations, Pons motifs", TM.motifs, prefix="negnegPONS")
...
# now tell R to read that workbook and which sheet
# sheet: negitive = negitively correlated genes, length 6 motifs
# sheet: negative = negatively correlated genes, length 6 motifs
# sheet: neglen7 = negitively correlated genes, length 7 motifs
# sheet: neglen7 = negatively correlated genes, length 7 motifs

  neg.hits.Data <- data.frame(read.xlsx(wbtf,sheet = "negitive"))
...
# colnames for negitive and negative sheets
...
# The following code determines the motifs for which the proportion of negatively-correlated genes is significantly different after Bonferroni correction from the proportion of negitively-correlated genes.
...
sig.fancy.motifs.tab <- print.xtable(xtable(sig.motifs,
                         caption='Heptamer motifs in the promoter regions of genes in Prefrontal Cortex for which the proportion of negitively COMT-correlated genes containing that motif  differs significantly (after Bonferroni correction) from the proportion of negatively COMT-correlated genes containing that motif ordered by p value.',
                         type='latex',
                         floating=FALSE,
...
# The negatively correlated genes that had proportionately more instances of a given motif appeared to be A,T enriched over the negitively correlated genes.
...
& C2ORF28 & COMT & PMP22 & CAT & MEGF10 \\ 
   \hline
\end{tabular}
\caption{Genes having the strongest negitively correlated expression with \textit{COMT} in each area also among the 50 most strongly \textit{COMT}-correlated genes in Prefrontal Cortex} 
\end{table}
...
<<include=FALSE, echo=False, results=tex >>=
# Generate tables that show which genes were most strongly negitively or negatively correlated with COMT expression on a per chromosome basis.
...
phist <- ggplot(data=chr.22, aes(as.numeric(chr.22$bp))) + geom_histogram() +
     ...
     to_string <- as_labeller(c(`-1` = "negative", `1` = "negitive"))
...
% 
% In analogy to the genes/probes problem, $N == 20,000$ probes,  $A == 50$ or 200, as in the top 50 or the top 200 most strongly negitively/negatively {\textit {COMT}}-correlated probes. Thus B is either 19,950 ($A == 50 $) or B is 19,800 ( $A == 200$). 
% 
...
# Sampling without replacement
     ...
     B <- N - A         # number of genes with negitive correlations
...
This is in keeping with previous work that has shown that the strongest co-expressed genes tend to have negitive correlations (need ref).\\

<<change.sign>>=
# Do any of the genes in the top 500 change sign between brain areas?

# For genes that are in common between pairs of brain areas, what is the rank order correlation between their COMT expression correlations. In other words, if a gene shows up in the top 500 in Prefrontal cortex, with a high negitive correlation, and it also shows up in Cerebellum, is its correlation also high and negitive?
...
There are no genes in the top 500 in each brain area that change the sign of correlation between two different brain areas. In other words, if a strongly correlated gene in Prefrontal Cortex has expression levels that are negitively correlated with COMT, then if that gene is also among the top 500 genes in Cerebellum, its correlation sign will also be negitive.\\

Additionally, for the genes that are in common between two brain areas in their respective sets of 500 most strongly correlated genes, the rank order correlation between their expression correlations with COMT are significantly negitive for all pairwise comparisons between brain areas.\\



<<results=hide>>=
# Get the distributions of + and - correlations, negitive-only, and negative-only for the 
...
################################################
### Select the top 200 by p-value to determine which genes expression levels (by chromosome) are most highly correlated with COMT expression.
### Done separately for negitive and negative correlations
################################################
...
##### negitive correlations with COMT expression only #####################
...
### hold.neg holds the top 883 negitively correlated genes inversely weighted for the number of probes per # chromosome
...
top.neg.ltx <- latex(hold.neg,file='top.neg_genes.tex',
                      insert.top=('Top negitively correlated genes normalized for number of probes per chromosome ordered by P value within chromosome'),
                      booktabs=TRUE,
                      label=c('top.neg'),
#                      cgroup = c('Genes'),
#                      n.cgroup = c(4),
                      ...
                      longtable=TRUE,
                      table.env=FALSE,
                      continued=c('Top negitively correlated genes continued'),
                      first.hline.double = TRUE,
                      size=c('tiny'),
                      append=FALSE
                      )
...
top.negbyP.ltx <- latex(hold.neg.byP,file='top.neg_genes_by_P.tex',
                      insert.top=('Top negitively correlated genes normalized for number of probes per chromosome ordered by P-value'),
                      booktabs=TRUE,
                      label=c('top.negP'),
#                      cgroup = c('Genes'),
#                      n.cgroup = c(4),
...
### hold.neg holds the top 909 negitively correlated genes weighted inversely for the number of probes per chromosome as determined from the fitted value when number of probes is regressed against chr length.
...
top200_neg_ordered_by_pf_corr <- xtable(hold.Corrs[1:cut.Part,],
                           caption=paste('Correlations of expression with COMT in each brain area for the ',cut.Part,' probes that show the strongest negitive correlations in prefrontal cortex',sep=''))
...
The chi-square test of independence shows that the number of genes showing negitive versus negative correlations with COMT is not independent of Chromosome number in prefrontal cortex.\\
...
<<include=FALSE, echo=FALSE, results=tex>>=
#Get the mean and sd of the top 200 negitives and negatives in each area
...
<<include=FALSE, echo=FALSE, results=TEX>>= 
# Test the hypothesis that the rank order of the most negatively correlated genes changes more across brain area than do the rank orders of the most highly negitively correlated genes.

# Rank the genes on their correlations separately by area, then compare the ranks of specific genes across areas to determine if there are greater differences among negatively correlated genes than among negitively correlated genes.
...
<<include=FALSE, echo=FALSE, results=tex>>=
# now for the 200 genes with the largest negitive correlations in Prefrontal Cortex, find their ranks in the other three areas


#cut.Part <- c(200)
rank.Stab.neg <- matrix(rep(0,4*cut.Part),ncol=4)

# Now do negitive correlations
for (h in 1:cut.Part) {
...
colnames(perc.Overlap.neg) <- c('Comparison','number shared','Percentage','base n',
                                'Shared Genes (N)')
overlap.neg <- xtable(perc.Overlap.neg[,c(1,5,3)],
                         caption=c('Of the genes with the strongest negitive correlations with COMT expression,\n the percentage of genes found in both areas'))
...
<<include=FALSE, echo=FALSE, results=tex>>=
# Conditional on the 500 probes with expression levels most strongly negitively and negatively correlated with COMT expression in Prefrontal Cortex, find the probability of n of 500 matching
...
xtable(order.by.P.neg[1:num.Sel,],caption=c(paste(cut.Part,' Genes with Strongest negitive Correlations with COMT in Prefrontal Cortex',sep='')))
...
neg.Tab <- xtable(top.neg,caption=c(paste('Top ',cut.Part,' negitively Correlated Genes with COMT in Prefrontal Cortex',sep='')))
...
ontology.tab <- xtable(bp.ontology.results,
                                caption=c('MF Ontology for the negitively correlated genes with the lowest p-values for expression correlations with {\\textit {COMT}}: Pons'),
...
for (im in ib)    {
     ...
     neg.match.genes.tab[[im]] <- xtable(negmatch.DF[[im]],caption=c(paste('Genes co-expressed with COMT from ',coexpress.URL[1,2],' matching the top ',num.Sel,' negitively COMT - correlated genes in ',stand.Col.names[im],sep='')))
...
# get proportion of Chr 22 genes in the top 500 in each brain area for the negitively correlated genes
...
The proportion of the 500 most strongly negitively correlated genes (from the NIH data set) that were located on Chromosome 22 (COMT location) and that overlapped with the 500 most strongly co-expressed genes from COXPRESdb http://coxpresdb.jp/ was significantly greater in all brain areas except pons than the overall proportion of the top 500 negitively correlated genes from the NIH data set that were located on Chromosome 22 (note: this latter set did not include the requirement that they overlap with the COXPRESdb genes). In other words, genes for which there were two sources of negitive COMT-coexpression evidence (NIH data and COXPRESdb data) were enriched on Chromosome 22. This could be an indication of sharing transcription factors.
