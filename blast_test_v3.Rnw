...
source('/Volumes/Macintosh_HD_3/genetics/genenetwork2/func_obj_size_Mb.R')  # objSize.Mb(object1)
...
# Add a comment column to the major.Are frames and the primary data frames to include information such as genes for which a LOCxxx or an ENTREZID were changed or discontinued by the NCBI Gene database.

lprim <- dim(major.Area[[2]])[1]
for (ih in 1:4)  {
     major.Area[[ih]]$comment <- c(rep('',lprim))
}
...
where.rentrez.symb.Loc <- match(rentrez.results.by.entrez$uid,major.Area[[area.now]]$ENTREZID[unlist(symb.LOC[[1]])])
...
#################################################################
  ...
  for (ih in 1:4)   {
       ...
       for (ig in 1:length(discon[[1]]))
  
          major.Area[[ih]]$comment[unlist(discon[[ih]])[ig]] <- comment.add
  
  }
  
  # row 4 HMCN2 replaces LOC392395 in cols 3,4, startpos 130.265742 into col 7,8
          # new entrez 256158 into cols 1,2 rentrez
  comp.avail.needed[2,c(1,2,3,4)] <- c('256158','256158','HMCN2','HMCN2')
  comp.avail.needed[2,c(7,8)] <- c(130.265742,130.265742)
