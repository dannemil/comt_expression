...
pre.Name <- c('/Volumes/Macintosh_HD_3/genetics/genenetwork2/mb_comt_plots_tables/') #pre-pended to pdf output files
...
histpandn <- paste(pre.Name,'mb_comt_histpandn.pdf',sep='')

@
\includegraphics[width=\textwidth]{\Sexpr{histpandn}}
\end{center}
\end{figure}
...
cisplot1pos <- paste(pre.Name,'cisplot1pos.pdf',sep='')

@

\includegraphics[width=\textwidth]{\Sexpr{cisplot1pos}}
\end{center}
\end{figure}
...
cisplot1neg <- paste(pre.Name,'mb_comt_cisplot1neg.pdf',sep='')

@
\includegraphics[width=\textwidth]{\Sexpr{cisplot1neg}}
\end{center}
\end{figure}
...
cisplot2pos <- paste(pre.Name,'mb_comt_cisplot2pos.pdf',sep='')

@

\includegraphics[width=\textwidth]{\Sexpr{cisplot2pos}}
\end{center}
\end{figure}
...
cisplot2neg <- paste(pre.Name,'mb_comt_cisplot2neg.pdf',sep='')

@

\includegraphics[width=\textwidth]{\Sexpr{cisplot2neg}}
\end{center}
\end{figure}
...
cisplotboth <- ('mb_comt_cisplotboth.pdf')

@

\includegraphics[width=\textwidth]{\Sexpr{cisplotboth}}
\end{center}
\end{figure}
...
probesvlength <- paste(pre.Name,'mb_comt_plotChrcountvsLength.pdf',sep='')

@
\includegraphics[width=\textwidth]{\Sexpr{probesvlength}}
\end{center}
\end{figure}
...
# Use Poisson regression to model the probe counts

model=glm(Probe.Count~1+Chromosome.Length,family=poisson(link=log),data=raw.Chr.count)
adjust.By.fit <- data.frame(pred=model$fitted)
...
top.200.pos <- order.by.P.pos[1:num.Top, ]
top200.pos.Count <- table(top.200.pos[,1])


## This scales the number of probes in the top 200 inversely by the fitted number of probes from the regression of number of probes by chromosome length. It also preserves the absolute number of probes found in the top 200 (~886)
fit.inverse.Scaled <- as.numeric(top200.pos.Count)/(adjust.By.fit[1:23,1]/sum(adjust.By.fit[1:23,1]))
fit.inverse.Scaled <- round((fit.inverse.Scaled*(sum(top200.pos.Count)/sum(fit.inverse.Scaled))),0)
...
# 
# num2get <- top200.pos.Count*scaled.Ratio$prop
...
hold.pos <- data.frame(CHR=rep(NA,sum(num2get2$n)),
                       ...
                       CORR=rep(NA,sum(num2get2$n))
)
hold.pos$GENE <- as.character(hold.pos$GENE)
hold.pos$CHR <- as.numeric(hold.pos$CHR)
hold.pos$P <- as.numeric(hold.pos$P)
hold.pos$CORR <- as.numeric(hold.pos$CORR)

### hold.pos holds the top 200 positively correlated genes inversely weighted for the number of probes per # chromosome
...
for (ig in 1:23) {
  ...
  hold.pos[lims.Count[1]:lims.Count[2],1:5] <-data.frame(subset(manhat.Posr,manhat.Posr$CHR == ig)[1:num2get2$n[ig],c(1:4,6)])
}

colnames(hold.pos) <- c('CHR','BP','P','GENE','CORR')
...
top.PosbyP.ltx
write.table(hold.pos.byP,file=paste(pre.Name,'mb_comt_top_pos_r_prefrontal.csv',sep=''),sep=', ',quote=FALSE)
...
top.200.neg <- order.by.P.neg[1:num.Top, ]
top200.neg.Count <- table(top.200.neg[,1])

scaled.Props <- as.numeric(full.Count)/num.Neg
scaled.top200.Props <- as.numeric(top200.neg.Count)/sum(as.numeric(top200.neg.Count))
...
fit.inverse.Scaled <- as.numeric(top200.neg.Count)/(adjust.By.fit[1:23]/sum(adjust.By.fit[1:23]))
fit.inverse.Scaled <- round((fit.inverse.Scaled*(sum(top200.neg.Count)/sum(fit.inverse.Scaled))),0)
...
hold.neg$CORR <- as.numeric(hold.neg$CORR)

### hold.neg will hold the top 200 positively correlated genes weighted inversely for the number of probes per chromosome as determined from the fitted value when number of probes is regressed against chr length.
...
for (ig in 1:23) {
  lims.Count[1] <- lims.Count[2] + 1
  lims.Count[2] <- lims.Count[1] + num2get2$n[ig] - 1
  hold.neg[lims.Count[1]:lims.Count[2],1:5] <-data.frame(subset(manhat.Negr,manhat.Negr$CHR == ig)[1:num2get2$n[ig],c(1:4,6)])
}

colnames(hold.neg) <- c('CHR','BP','P','GENE','CORR')
...
top.NegbyP.ltx
write.table(hold.neg.byP,file=paste(pre.Name,'mb_comt_top_neg_r_prefrontal.csv',sep=''),sep=',',quote=FALSE)
...
#                      caption=paste('Chromosome ',as.character(my.Genes[[j]][1,1]),sep=''),
                      label=c('genecount'),
                      colheads=c('Chromosome','Positive r','Negative r'),
                      na.blank = TRUE,
                      vbar = FALSE,
...
hist1 <- paste(pre.Name,'mb_comt_expr_Var_hist+r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{hist1}}
}
\qquad
...
hist2 <- paste(pre.Name,'mb_comt_corr_Var_hist+r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{hist2}}
}
\qquad
...
hist3 <- paste(pre.Name,'mb_comt_expr_Var_hist-r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{hist3}}
}
\qquad
\subfloat[Histogram of sample correlations -r only]{
...
hist(comt.Meannegr$corr,xlab=c('Sample correlation negative r only'),
    ...
    col='gray90',
    25 
#    xlim = rev(range(comt.Meannegr$corr))
    )

hist4 <- paste(pre.Name,'mb_comt_corr_Var_hist-r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{hist4}}
}
...
hist(-log10(manhat.Posr$P),20,xlab=c('-log10(p-value)'),
     main=c('Positive COMT-correlations Only'),
     col='gray90',
     20
     )

histp1 <- paste(pre.Name,'mb_comt_minuslogP+r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{histp1}}
}
\qquad
\subfloat[Histogram of -log10(pval) -r only]{
...
hist(-log10(manhat.Negr$P),xlab=c('-log10(p-value)'),
    main=c('Negative COMT-correlations Only'),
    col='gray90',
    20
    )

histp2 <- paste(pre.Name,'mb_comt_minuslogP-r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{histp2}}
}
...
bar1 <- paste(pre.Name,'mb_comt_plotgenesperchrom+r.pdf',sep='')

@

\includegraphics[width=0.4\textwidth]{\Sexpr{bar1}}
}
\qquad
\subfloat[Barchart: Number of Genes Per Chromosome -r Only]{
...
bar2 <- paste(pre.Name,'mb_comt_plotgenesperchrom-r.pdf',sep='')
@

\includegraphics[width=0.4\textwidth]{\Sexpr{bar2}}
}
\end{center}
\end{figure}
...
scat1 <- paste(pre.Name,'mb_comt_corr_expr_posr.pdf',sep='')

@

\includegraphics[width=0.4\textwidth]{\Sexpr{scat1}}
}
\qquad
\subfloat[Scatterplot Expression Level vs. Correlation (+r) only]{
...
scat2 <- paste(pre.Name,'mb_comt_corr_expr_negr.pdf',sep='')

@

\includegraphics[width=0.4\textwidth]{\Sexpr{scat2}}
}
\end{center}
\end{figure}
...
manh1 <- paste(pre.Name,'mb_comt_Manhatt_posr.pdf',sep='')

@

\includegraphics[width=0.8\textwidth]{\Sexpr{manh1}}
\end{center}
\end{figure}
...
manh2 <- paste(pre.Name,'mb_comt_Manhatt_negr.pdf',sep='')
@


\includegraphics[width=0.8\textwidth]{\Sexpr{manh2}}
% }
\end{center}
\end{figure}
...
manh3 <- paste(pre.Name,'mb_comt_Manhatt_bothr.pdf',sep='')
@
...
p1
ggmanp1 <- paste(pre.Name,'mb_comt_ggmanp1.pdf',sep='')

@

%\includegraphics[width=0.8\textwidth]{\Sexpr{ggmanp1}}
\end{center}
\end{figure}
...
p2
ggmanp2 <- paste(pre.Name,'mb_comt_ggmanp2.pdf',sep='')

@

%\includegraphics[width=0.8\textwidth]{\Sexpr{ggmanp2}}
\end{center}
\end{figure}
...
p3
ggmanp3 <- paste(pre.Name,'mb_comt_ggmanp3.pdf',sep='')

@

%\includegraphics[width=0.8\textwidth]{\Sexpr{ggmanp3}}
\end{center}
\end{figure}
