...
# For the all genes set only split from -40 to +10 (960 to 1010)     
     for (gh in 1:length(proms.all[[im]]))   {
          ...
          fasta.by.lett_all.genes[[im]][gh] <- strsplit(substring(as.character(proms.all[[im]][gh]),960,1010),NULL)
...
# For positively COMT-correlated genes set only split from -40 to +10 (960 to 1010)     
     for (gh in 1:length(proms.pos[[im]]))   {
          ...
          fasta.by.lett.pos[[im]][gh] <- strsplit(substring(as.character(proms.pos[[im]][gh]),960,1010),NULL)
...
# For negatively COMT-correlated genes set only split from -40 to +10 (960 to 1010)     
     for (gh in 1:length(proms.neg[[im]]))   {
          ...
          fasta.by.lett.neg[[im]][gh] <- strsplit(substring(as.character(proms.neg[[im]][gh]),960,1010),NULL)
...
all.area.PWM.bground <- rbind(all.PWM.long[[1]],all.PWM.long[[2]],
                            all.PWM.long[[3]],
                            all.PWM.long[[4]])
...
pos.genes.PWM.near.TSS <- lapply(pos_genes.lett.freq[ib], function(x,ib) data.frame(x), ib )

pos.PWM.long <- lapply(pos.genes.PWM.near.TSS[ib], function(x,ib) stack(x), ib)

for (im in ib)    {
     
     pos.PWM.long[[im]]$loci <- as.integer(c(rep(-40:10,each=4)))
     pos.PWM.long[[im]]$ntide <- c(rep(c('A','C','G','T'),length(subseq)))

}

pos.area.PWM.bground <- rbind(pos.PWM.long[[1]],all.PWM.long[[2]],
                            pos.PWM.long[[3]],
                            pos.PWM.long[[4]])
pos.area.PWM.bground$area <- c(rep(stand.Col.names.aug[1:4],each=204))

######### negative
# long form

neg.genes.PWM.near.TSS <- lapply(neg_genes.lett.freq[ib], function(x,ib) data.frame(x), ib )

neg.PWM.long <- lapply(neg.genes.PWM.near.TSS[ib], function(x,ib) stack(x), ib)

for (im in ib)    {
     
     neg.PWM.long[[im]]$loci <- as.integer(c(rep(-40:10,each=4)))
     neg.PWM.long[[im]]$ntide <- c(rep(c('A','C','G','T'),length(subseq)))

}

neg.area.PWM.bground <- rbind(neg.PWM.long[[1]],
                              neg.PWM.long[[2]],
                            neg.PWM.long[[3]],
                            neg.PWM.long[[4]])
neg.area.PWM.bground$area <- c(rep(stand.Col.names.aug[1:4],each=204))
...
params.plot <- data.frame(titlen=c("15998 Genes",
                         ...
                         fnamesa=c("pwm_all_genes_pf",
                                  "pwm_pos_genes_pf",
                                  "pwm_neg_genes_pf"),
                         fnamesb=c("pwm_all_genes_by_ntide_pf",
                                  "pwm_pos_genes_by_ntide_pf",
                                  "pwm_neg_genes_by_ntide_pf")
                          )
...
plotn[[pj]] <- ggplot(data=neg.area.PWM.bground[1:204,], aes(x=loci,y=values,color=ntide)) +
          geom_line(size=1) +
          theme_classic() +
          labs(title='Site-specific nucleotide proportions (TSS = 0)',
               x = 'Position',
               y = 'Proportion',
               subtitle=c(paste("Prefrontal Cortex: ",
...
### By nucleotide
plotb <- list()
all.area.PWM.bground$ntide <- factor(all.area.PWM.bground$ntide, levels = c("C","G",
                                                                            "A","T"))

pos.area.PWM.bground$ntide <- factor(pos.area.PWM.bground$ntide, levels = c("C","G",
                                                                            "A","T"))

neg.area.PWM.bground$ntide <- factor(neg.area.PWM.bground$ntide, levels = c("C","G",
                                                                            "A","T"))

pj <- c(3)

plotb[[pj]] <- ggplot(data=neg.area.PWM.bground[1:204,], aes(x=loci,y=values,color=ntide)) +
...
pwms.site.pos <- list()
     ...
     for (im in ib) {
          ...
          for (dk in 1:length(proms.pos[[im]])) { # genes
               ...
               aug.pwms[dk,1] <- as.numeric(sum(pwm.by.site[dk,code.base[unlist(fasta.by.lett.pos[[im]][dk])[sseq], ]]))
               aug.pwms[dk,2] <- proms.pos.entrez[[im]][dk]
               aug.pwms[dk,3] <- proms.pos.strand[[im]][dk]
               
          }
               
               pwms.site.pos[[im]] <- aug.pwms
...
for (im in ib) {
     ...
     out.t.test[[im]] <- t.test(pwms.site.pos[[im]]$V1,pwms.site.neg[[im]]$V1,var.equal = TRUE)
...
#     obs.freq <- gene.samp_sizes.pos[[1]]*pwm.by.site[,1:4]
     exp.freq <- gene.samp_sizes.pos[[1]]*pwm_neg_comt_genes[[1]][,1:4]
...
#     out.pvals[,1] <- c(-40:10)
     ...
     sorted.pvals <- out.pvals[order(out.pvals[,2]),]
