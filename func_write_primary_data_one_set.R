...
write.primary.one <- function(ax,namex)  {
...
# Using the extension, .txt, allows these to be imported into excel so that the column with gene Symbols can be set to text, preventing Excel from converting gene Symbols like MARCH1 to date format.
     success.write <- is.null(try(write.csv(as.data.frame(ax),paste("/Volumes/Macintosh_HD_3/genetics/genenetwork2/comt_correlations_20000_",namex,"_augmentedNew.csv.txt",sep=""),row.names=FALSE)))
# # # # # #
