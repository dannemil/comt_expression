...
fname <- c("/Volumes/Macintosh_HD_3/genetics/genenetwork2/comt_correlations_by_genes_all_areas_50.Rnw")
...
prefront.Meanposr <- data.frame(mean=NA,corr=NA,chr=NA,gene=NA,bp=NA,pval=NA)
prefront.Meannegr <- data.frame(mean=NA,corr=NA,chr=NA,gene=NA,bp=NA,pval=NA)
prefront.Meanbothr <- data.frame(mean=prefront.Data$Mean,corr=prefront.Data$Sample.r,chr=prefront.Data$loc,
                             gene=prefront.Data$Symbol,bp=prefront.Data$bp,pval=prefront.Data$Sample.p.r.)
prefront.Meanbothr$corsign <- as.factor(sign(prefront.Data$Sample.r))
prefront.Meanbothr$chr[which(prefront.Meanbothr$chr == '   Un')] <- c(NA)
### Get rid of "Un/NA" chromosome rows
prefront.Meanbothr2 <- data.frame(na.omit(prefront.Meanbothr))
rm(prefront.Meanbothr)
prefront.Meanbothr <- prefront.Meanbothr2   # <--- This holds pos and neg corr data
...
prefront.Meanposr <- data.frame(mean=posrmeans.pf,corr=posrcorrs.pf,chr=posrchr.pf,gene=posgene.pf,
                            bp=posbp.pf,pval=pospval.pf)
prefront.Meannegr <- data.frame(mean=negrmeans.pf,corr=negrcorrs.pf,chr=negrchr.pf,gene=neggene.pf,
                            bp=negbp.pf,pval=negpval.pf)
...
# prefrontal
dim.Area$len[1] <- dim(prefront.Data)[1]
dim.Area$wid[1] <- dim(prefront.Data)[2]
...
prefront.Ess <- data.frame(area=rep('prefront',dim.Area$len[1]),
                      loc=prefront.Data$loc,
                      Symbol=prefront.Data$Symbol,
                      Sample.r=prefront.Data$Sample.r,
                      Sample.p.r=prefront.Data$Sample.p.r.,
                      Record=prefront.Data$Record)
...
The chi-square test of independence shows that the number of genes showing positive versus negative correlations with COMT is not independent of Chromosome number.\\
...
# p(k <= 186 minuses) with Binomial(N=17482, p = 0.5094)

cumul.d.minus <- sum(d.minus$prob[1:(obs.Minus.22 + 1)])
...
Alternatively, the probability of observing \Sexpr{obs.Minus.22} negative correlations out of a sample of \Sexpr{counts.22} correlations by chance given a true proportion of negative correlations of \Sexpr{p.minus} is \Sexpr{round(cumul.d.minus,9)}.\\ 

<<include=FALSE, echo=FALSE, fig=TRUE, pdf=TRUE>>=
gold.R <- c(1.61803398875)

vcut <- data.frame(x1=186.5, x2=186.5, y1=-0.00075, y2=0.02)
p.low.d.minus <- subset(d.minus,d.minus$num.minus < 187)
...
minus.plot <- ggplot() +
          ...
          annotate("text", x = 125,
                   y = 0.005,
                   label = c('p(k < 187) = 3.908e-7'),
                   color="black",
                   size=3.0)

file1.pdf <- c('p_binomial_comt_s.pdf')
...
<<include=FALSE, echo=FALSE, results=TEX>>= 
# Test the hypothesis that the rank order of the most negatively correlated genes changes more across brain area than do the rabnk orders of the most highly positively correlated genes.
...
% {\bfseries {Using the hypergeometric distribution to test for randomness in the number of probes shared in common between the reference area and anoter area.}}\\
% 
...
# Note, the probability in the table should really be the cumulative probability of finding k or more matches
...
for (i in 1:6) {
          ...
          genes.In.common$logp.valpos[i] <- round(log10(as.numeric(mpfr(1.0 - sum(prob.N[1:(perc.Overlap.pos[i,2]+1),1]),precBits=50))),1)
          genes.In.common$logp.valneg[i] <- round(log10(as.numeric(mpfr(1.0 - sum(prob.N[1:(perc.Overlap.neg[i,2]+1),1]),precBits=50))),1)
...
# 
# biomaRt::select(gene.DB,keys=as.character(temp.Gene),columns=c('GENENAME','SYMBOL','ENTREZID'                    ),keytype=c('SYMBOL'))
