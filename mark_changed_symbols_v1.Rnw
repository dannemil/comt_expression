...
# Create table of number of identical symbols for pairs of areas in orig. and pre-processed data
for (ij in 1:3) {
     
     common.Symb[ij,4] <- length(unique(orig.major.Area[[ij]]$Symbol))
     common.Symb[4,4] <- length(unique(orig.major.Area[[ij]]$Symbol))
     not.common.Symb[ij,4] <- length(unique(orig.major.Area[[ij]]$Symbol))
     not.common.Symb[4,4] <- length(unique(orig.major.Area[[ij]]$Symbol))
     
     common.Symb[ij,8] <- length(unique(major.Area[[ij]]$Symbol))
     common.Symb[4,8] <- length(unique(major.Area[[ij]]$Symbol))
     not.common.Symb[ij,8] <- length(unique(major.Area[[ij]]$Symbol))
     not.common.Symb[4,8] <- length(unique(major.Area[[ij]]$Symbol))
     
     
     for (ik in (ij+1):4)   {
          
          common.Symb[ij,(ik-1)] <- length(intersect(unique(orig.major.Area[[ij]]$Symbol),
                                                     unique(orig.major.Area[[ik]]$Symbol)))
          common.Symb[ij,(ik-1 + 4)] <- length(intersect(unique(major.Area[[ij]]$Symbol),
                                                     unique(major.Area[[ik]]$Symbol)))
     
          not.common.Symb[ij,(ik-1)] <- length(setdiff(unique(orig.major.Area[[ij]]$Symbol),
                                                     unique(orig.major.Area[[ik]]$Symbol)))
          not.common.Symb[ij,(ik-1 + 4)] <- length(setdiff(unique(major.Area[[ij]]$Symbol),
                                                     unique(major.Area[[ik]]$Symbol)))
     } # end inner brain area loop

} # end outer brain area loop

row.names(common.Symb) <- stand.Col.names.lc
colnames(common.Symb) <- c(rep(c(stand.Col.names.lc[2:4],'Unique'),2))
row.names(not.common.Symb) <- stand.Col.names.lc
colnames(not.common.Symb) <- c(rep(c(stand.Col.names.lc[2:4],'Unique'),2))

symbols.in.common.tab <- xtable(common.Symb,
                                caption=c('Gene symbols in original, unprocessed data (columns 1 to 3) and in the preprocessed data (columns 4 to 6) shared by pairs of brain areas.'))

symbols.not.shared.tab <- xtable(not.common.Symb,
                                caption=c('Gene symbols in original, unprocessed data (columns 1 to 3) and in the preprocessed data (columns 4 to 6) not shared by pairs of brain areas.'))
...
##### Search for date-like symbols in major.Area to back substitute into the original data files which converted them to numbers after brining them in as dates.


# Search for date-like symbols to back substitute into the original data files that formatted them as dates and then converted them to numbers.

# Only the MARCHx need to be replaced in the original files because the SEPTx symbols appeared as Septin x not as dates in thopse files.
date.like.symbols <- list()

for (im in ib)   {
     date.like.symbols[[im]] <- 
          major.Area[[im]]   %>%
                    filter(grepl('^MARCH',Symbol))
     
}

orig.major.Area[[1]]$Symbol[c(unlist(date.like.symbols[[1]]$Index))]
major.Area[[1]]$Symbol[c(unlist(date.like.symbols[[1]]$Index))]

# Now replace the symbols in the original data.frames

for (im in ib)   {
     
     orig.major.Area[[im]]$Symbol[c(unlist(date.like.symbols[[im]]$Index))] <- major.Area[[im]]$Symbol[c(unlist(date.like.symbols[[im]]$Index))]
     
     
}

# create new data frames to hold the original data

orig.prefront.Data <- orig.major.Area[[1]]
orig.cbell.Data <- orig.major.Area[[2]]
orig.tempor.Data <- orig.major.Area[[3]]
orig.pons.Data <- orig.major.Area[[4]]
@
<< write_orig_data_out >>=
##### Note: these will be new individual files with the prefix orig.



#### >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> NEED AREA FILENAME HERE 
success.of.write <- write.orig.one(orig.pons.Data,'pons')
