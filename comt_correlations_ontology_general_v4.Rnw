...
<<include=FALSE, echo=FALSE, results=hide>>=
#Because some genes have more than one illumina probe, that gives them an advantage in apriori probability terms of ending up in the top (or bottom) 50 or 200 correlated genes. One solution is to use the correlation that corresponds to the maximum expression level for each gene that has more than one probe. Note, the maximum expression levels of both the set of positively correlated probes and the set of negatively correlated probes shpould be used. There will be a maximum for the + correlations and a maximum for the - correlations (assuming that the complete set of probes has probes that have both + and - correlations with COMT.
...
# First, average correlations
# average correlations across areas
...
avg.corr <- matrix(rep(0,dim(area.colwise)[1],ncol=1))

for (l in 1:dim(area.colwise)[1]) {
     n.part <- c(0)
     for (j in 1:4)   {
     
          n.part <- n.part + n.subj[j]
     avg.corr[l] <- avg.corr[l] + (n.subj[j] - 1)*(area.colwise[l,(j+8)] + area.colwise[l,(j+8)]*(1 - area.colwise[l,(j+8)]^2)/(2*(n.subj[j] - 3)))
     
#hold.Corrs$avgCorr[l] <- sum.corr[l]
     }
     
     avg.corr[l] <- avg.corr[l]/(n.part - ki)
}

# Now calculate variance

var.corr <- rowSums((area.colwise[,9:12] - avg.corr)^2)/(ki - 1)

# Now calculate the coefficient of variation for the correlation coefficient

coeff.Var <- sqrt(var.corr)/abs(avg.corr)

# Now add coluns to area.colwise

area.colwise$avg <- avg.corr
area.colwise$varnc <- var.corr
area.colwise$coeffVar <- coeff.Var

# Order by var
area.colwise <- area.colwise[order(area.colwise$varnc,decreasing=TRUE),]
...
<<>>=
# Dendrogram plots of the relastions between the 8 shared genes on Chr 5
# For MR
...
<< results=tex>>=
# Ontology on 50 most strongly negatively correlated genes in ********Certebellum***********
...
symbols.top.bottom.20 <- data.frame(pfpos=rep(0,num.Ont),
                                    pfneg=rep(0,num.Ont),
                                    cbpos=rep(0,num.Ont),
                                    cbneg=rep(0,num.Ont),
                                    tmpos=rep(0,num.Ont),
...
symbols.top.bottom.20[1:20,(v+(w-1))] <- noquote(area.onegene.List[[(v+(w-1))]]$Symbol[lims.Ont[w,1]:lims.Ont[w,2]])
