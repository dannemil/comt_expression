...
Inr.hits.motif1.long <- data.frame(loci=c(unlist(motif.hits.loci.all[[1]][[1]])),
                             motif=c(rep('B-W-C-A-B-W',length(unlist(motif.hits.loci.all[[1]][[1]])))
                                     ))
...
# Trasform the ftable into a 4 x 3 x 2 array
...
ngenes <- c(1:length(proms.pos[[1]]))
tab.DNA.pos.pf <- lapply(proms.pos[[1]][ngenes], function(x,ngenes) as.character(unlist(x)[990:1010]), ngenes)

dna.disp.pos.pf <- xtable(matrix(noquote(as.character(tab.DNA.pos.pf)),ncol=1))
